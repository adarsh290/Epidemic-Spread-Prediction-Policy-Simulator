...
def download_data():
    ...
    jhu_long = jhu_long.dropna(subset=['Date']).sort_values(by=['Country', 'Date']).reset_index(drop=True)
...
def process_and_merge():
    ...
    google_cols = ['Country', 'Date', 'retail_and_recreation_percent_change_from_baseline', 
...
def feature_engineering():
    ...
    df['WoW_Growth'] = np.where(df['Cases_7d_avg_last_week'] > 0, df['Cases_7d_avg'] / df['Cases_7d_avg_last_week'], 0)
...
def train_model():
    ...
    features = ['retail_and_recreation_percent_change_from_baseline_Lag_14d',
...
if __name__ == "__main__":
    ...
    logger.info("Pipeline script ready. Uncomment functions in __main__ to run specific steps.")
